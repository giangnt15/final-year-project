...
function AuthorEdit(props) {
    ...
    const { loading: gettingAuthor, refetch: refetchAuthor } = useQuery(GET_AUTHOR, {
        onError() {
        ...
        onCompleted(data) {
            if (data.getAuthor) {
                setInputs(prev => ({
                    ...prev,
                    realName: data.getAuthor.realName,
                    pseudonym: data.getAuthor.pseudonym,
                    description: data.getAuthor.description,
                }))
            }
        },
        variables: {
    ...
    const [updateAuthor, { loading: updatingAuthor }] = useMutation(UPDATE_AUTHOR, {
        onError() {
                    ...
                    <Panel header={<span><i className="fa fa-book m-r-12"></i>Sách</span>} key="2" showArrow={false}>
                        <Fragment><div className="actions">
                            ...
                            <Button disabled={selectedRowKeys.length === 0}
                                ...
                                className="m-l-8" type="danger"><span><DeleteOutlined /> Xóa khỏi tác giả</span></Button>
                        </div>
                            <br />
